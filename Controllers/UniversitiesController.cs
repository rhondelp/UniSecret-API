...
public class UniversitiesController : ControllerBase
{
    ...
    {
        var cached =
            await _cache.GetAsync<List<UniversityDto>>(
                CacheKey,
                cancellationToken);
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<UniversityDto>> GetUniversity(
        int id,
        CancellationToken cancellationToken = default)
    {
        ...
        var cached =
            await _cache.GetAsync<UniversityDto>(
                cacheKey,
                cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (domainExists)
        {
            return BadRequest(
                ...
                {
                    message =
                        "A university with this domain already exists."
                });
        }
